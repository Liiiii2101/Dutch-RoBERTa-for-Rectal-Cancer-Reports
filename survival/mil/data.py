...
class BagDataset(Dataset):
    ...
    def __getitem__(self, index: int) -> Tuple[torch.Tensor, int]:
        ...
        feats = []
        #feats.append(np.load(f"/data/groups/beets-tan/l.cai/rectal_nlp/radiology_npy/train/{self.ids[idx]}.npy"))
        for bag_file in self.bags[index]:
            #with h5py.File(bag_file, 'r') as f:
            feats.append(torch.from_numpy(np.load(bag_file)))
...
class DataSequence(torch.utils.data.Dataset):
    ...
    def get_batch_data(self, idx):
        ...
        if len(modality) == 1:
            if self.train:
                return np.load(f"/data/groups/beets-tan/l.cai/rectal_nlp/{modality[0].lower()}_npy/train/{self.ids[idx]}.npy")
            else:
                return np.load(f"/data/groups/beets-tan/l.cai/rectal_nlp/{modality[0].lower()}_npy/test/{self.ids[idx]}.npy")
        else:
            feature = []
            if self.train:
                if os.path.exists(f"/data/groups/beets-tan/l.cai/rectal_nlp/radiology_npy/train/{self.ids[idx]}.npy"):
                    radiology = np.load(f"/data/groups/beets-tan/l.cai/rectal_nlp/radiology_npy/train/{self.ids[idx]}.npy")
                else:
                    #print('radiology 0')
                    radiology = np.zeros((1,768))
                feature.append(radiology)
                if os.path.exists(f"/data/groups/beets-tan/l.cai/rectal_nlp/pathology_npy/train/{self.ids[idx]}.npy"):
                    #print('pathology not 0')
                    pathology = np.load(f"/data/groups/beets-tan/l.cai/rectal_nlp/pathology_npy/train/{self.ids[idx]}.npy")
                else:
                    #print('pathology 0')
                    pathology = np.zeros((1,768))
                #feature.append(np.zeros((1,768)))
                # if os.path.exists(f"/data/groups/beets-tan/l.cai/rectal_nlp/endoscopy_npy/train/{self.ids[idx]}.npy"):
                #     endoscopy = np.load(f"/data/groups/beets-tan/l.cai/rectal_nlp/endoscopy_npy/train/{self.ids[idx]}.npy")
                # else:
                #     endoscopy = np.zeros((1,768))
                # feature.append(endoscopy)
            else:
                if os.path.exists(f"/data/groups/beets-tan/l.cai/rectal_nlp/radiology_npy/test/{self.ids[idx]}.npy"):
                    radiology = np.load(f"/data/groups/beets-tan/l.cai/rectal_nlp/radiology_npy/test/{self.ids[idx]}.npy")
                else:
                    radiology = np.zeros((1,768))
                feature.append(radiology)
                if os.path.exists(f"/data/groups/beets-tan/l.cai/rectal_nlp/pathology_npy/test/{self.ids[idx]}.npy"):
                    pathology = np.load(f"/data/groups/beets-tan/l.cai/rectal_nlp/pathology_npy/test/{self.ids[idx]}.npy")
                else:
                    pathology = np.zeros((1,768))
                #feature.append(pathology)
                # if os.path.exists(f"/data/groups/beets-tan/l.cai/rectal_nlp/endoscopy_npy/test/{self.ids[idx]}.npy"):
                #     endoscopy = np.load(f"/data/groups/beets-tan/l.cai/rectal_nlp/endoscopy_npy/test/{self.ids[idx]}.npy")
                # else:
                #     endoscopy = np.zeros((1,768))
                # feature.append(endoscopy)
    ...
    def __getitem__(self, idx):
        ...
        feature = self.get_batch_data( idx)
        #print(feature.shape)
        os = float(self.df[self.df['id']==self.ids[idx]][f'{goal} Time'])#][['OS Time', 'OS_event']]]
        os_e = int(self.df[self.df['id']==self.ids[idx]][f'{goal}_event'])
        #print(self.ids, os, os_e)
        return {
            'features': torch.tensor(feature, dtype=torch.float32),
            'labels': np.array([os, os_e]),
            'id': self.ids[idx]
        }
